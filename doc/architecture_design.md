...
- [ ] 포트별 링크드 리스트 큐 매니져 구조
- [ ] 포트별 엔트리 풀 분리 및 Entry ID 기반 소유권 관리
- [ ] axis_switch와 큐 매니져 간 인터페이스 상세 설계
- [ ] 패킷 stitching 메커니즘 상세화 (Entry ID 기반)
- [ ] Ingress 크레딧 예약 및 관리 시스템
- [ ] Egress 백프레셔 제어 (링크드 리스트 길이 기반)
- [ ] 큐별 크레딧 계산 및 업데이트 로직
- [ ] 라우팅 성능 최적화 방안
- [ ] 메모리 풀 크기 결정 알고리즘 (ingress/egress 분리)
- [ ] Entry ID 기반 디버깅 및 추적 시스템
