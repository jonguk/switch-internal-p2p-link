/******************************************************************************
 * Working Simple Error Injection Test
 * 
 * SystemC 문법을 올바르게 사용한 간단한 에러 테스트입니다.
 ******************************************************************************/
...
// =============================================================================
// Working Error Test Module
// =============================================================================

SC_MODULE(working_error_test) {
    ...
    sc_signal<ErrorType> current_error_type;
    
    // Statistics
    error_statistics stats;
    
    SC_CTOR(working_error_test) : clk("clk", 10, SC_NS) {
        ...
        current_error_type.write(ErrorType::NONE);
    }
    ...
    void test_crc_error() {
        ...
        current_error_type.write(ErrorType::CRC_ERROR);
    ...
    void test_invalid_port() {
        ...
        current_error_type.write(ErrorType::INVALID_PORT);
    ...
    void test_protocol_violation() {
        ...
        current_error_type.write(ErrorType::PROTOCOL_VIOLATION);
    ...
    void test_timeout_error() {
        ...
        current_error_type.write(ErrorType::TIMEOUT_ERROR);
    ...
    void error_detector_process() {
        ...
        ErrorType err_type = current_error_type.read();
        ...
        if (data_valid.read() && err_type != ErrorType::NONE) {
            ...
            if (error_detected) {
                ...
                // Error flag will be cleared by next clock edge
            }
        } else {
            data_error.write(false);
        }
...
int sc_main(int argc, char* argv[]) {
    std::cout << "=== Working Simple Error Injection Test ===" << std::endl;
    
    // Create testbench
    working_error_test tb("working_error_test");
    
    // Run simulation
    sc_start();
    
    std::cout << "=== Working Error Test Completed ===" << std::endl;
    return 0;
}